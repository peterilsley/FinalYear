...
%s=fstate(in);
%S=jacobian(s,in);
%THEN ITERATE FROM THIS STEP ONWARDS IN PARALLEL?
% x1=x(1,1)
% x2=x(2,1)
% x3=x(3,1)
% subs(S)
...
n=numel(x);
m=numel(z);
A=zeros(m,n, 'gpuArray');
h=n*eps;%why do they multiply by n*eps
...
A=gather(A);%could reduce having to do this if pipelining is done
z=gather(z);